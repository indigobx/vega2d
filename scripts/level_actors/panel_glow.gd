...
func interact() -> void:
  $GlowSprite2D.region_rect = Rect2(32, 0, 32, 32)
  $Timer.start(0.1)
  if call_node and callback and call_node.has_method(callback):
    call_node.call_deferred("callv", callback, callback_parameters)
    await $Timer.timeout
    $GlowSprite2D.region_rect = Rect2(0, 32, 32, 32)
  else:
    $GlowSprite2D.region_rect = Rect2(32, 32, 32, 32)
  $Timer.start(0.5)
  await $Timer.timeout
  $GlowSprite2D.region_rect = Rect2(0, 0, 32, 32)
