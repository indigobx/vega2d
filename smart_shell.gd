...
@export var start_impulse: float = 100
@export var engine_impulse: float = 15
...
func _physics_process(delta: float) -> void:
  ...
  if !target_hit and timer <= accel_timer:
    apply_impulse(Vector2(engine_impulse, 0).rotated(rotation))
